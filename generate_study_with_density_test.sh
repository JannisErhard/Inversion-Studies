 ...
 then
    ...
    do
      ...
      do
          ...
          sed "s/THR_DUMMY/$thr/g ; s/CHARGEDUMMY/$Charge/g; s/SYS_DUMMY/$Sys/g ; s/SPINDUMMY/$Unpaired/g; s/REFERENCEDUMMY/$E_Referenz/g; s/OEP_BASISDUMMY/$OEP_Basis/g ; s/AO_BASISDUMMY/$AO_Basis/g; s/MAJOR_DUMMY/"$Major"/g ; s/MINOR_DUMMY/"$Minor"/g ;"  input_template_1 > "$Element"/"$name"/"$sub_name"/input_1
          sed "s/THR_DUMMY/$thr/g ; s/CHARGEDUMMY/$Charge/g; s/SYS_DUMMY/$Sys/g ; s/SPINDUMMY/$Unpaired/g; s/REFERENCEDUMMY/$E_Referenz/g; s/OEP_BASISDUMMY/$OEP_Basis/g ; s/AO_BASISDUMMY/$AO_Basis/g; s/MAJOR_DUMMY/"$Major"/g ; s/MINOR_DUMMY/"$Minor"/g ;"  input_template_2 > "$Element"/"$name"/"$sub_name"/input_2
      done
    done
  done
done 
